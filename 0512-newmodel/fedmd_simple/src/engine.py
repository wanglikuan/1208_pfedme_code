    ...
    for iter in range(epoch):
        ...
        for batch_idx, (images, labels) in enumerate(train_dataloader):
            ...
            if with_softmax:
                ...
                # 重新定义损失函数，使用softmax，MSEloss #
                Tsoftmax = nn.Softmax(dim=1)
                #加入温度系数T#
                output_logit = Tsoftmax(log_probs.float()/5)
                ##
...
def predict(model,dataarray,device):
    ...
    with torch.no_grad():
        for ind in range(0,len(dataarray),bs):
            ...
            Tsoftmax = nn.Softmax(dim=1)
            #加入温度系数T#
            output_logit = Tsoftmax(logit.float()/5)
            #
    ...
    def collaborative_training(self):
        ...
        while True:
            ...
            for index, model in enumerate(self.collaborative_parties):
                ...
                with open('./result/real_noniid_normal_softmaxtest_T5.txt', 'a') as f:
                    f.write('{}\t{}\t{}\n'.format(r, index, acc))
            r += 1
            if r > self.N_rounds:
                break
