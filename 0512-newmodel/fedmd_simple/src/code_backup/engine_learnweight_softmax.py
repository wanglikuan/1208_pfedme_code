    ...
    for iter in range(epoch):
        ...
        for batch_idx, (images, labels) in enumerate(train_dataloader):
            ...
            if with_softmax:
                ...
                #加入温度系数T#
                output_logit = Tsoftmax(log_probs.float()/10)
                ##
...
def get_models_logits(raw_logits, weight_alpha, N_models): #raw_logits为list-np；weight为tensor；
    ...
    # 更新 raw_logits
    for self_idx in range(N_models): #对每个model计算其teacher的logits加权平均值
        weight_tmp = torch.zeros(N_models-1)
        idx_count = 0
    ...
    def collaborative_training(self):
        ...
        while True:
            ...
            for index, model in enumerate(self.collaborative_parties):
                ...
                dataloader =  DataLoader(data_new(X_test,y_test), batch_size=32,
                                          shuffle=True,
                ...
                with open('./result/newdata_learnweightsoftmax_label10_10_test2_T10_lr50.txt', 'a') as f:
                    f.write('{}\t{}\t{}\n'.format(r, index, acc))
