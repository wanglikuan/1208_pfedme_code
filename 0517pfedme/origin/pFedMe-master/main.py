...
def main(dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
    ...
    for i in range(times):
        ...
        if(model == "cnn"):
            if(dataset == "Mnist"):
                #model = newNet().to(device), model
                model = CNNTarget_mnist().to(device), model
            if(dataset == "EMnist"):
                #model = newNet().to(device), model
                model = CNNTarget_mnist().to(device), model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="Cifar10", choices=["Mnist", "EMnist","Synthetic", "Cifar10"])
    ...
    parser.add_argument("--algorithm", type=str, default="pFedMe",choices=["pFedMe", "PerAvg", "FedAvg","newPerAvg"]) 
