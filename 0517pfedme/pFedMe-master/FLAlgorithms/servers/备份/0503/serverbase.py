...
class Server:
    def __init__(self, device, dataset,algorithm, model, batch_size, learning_rate ,beta, lamda,
        ...
        self.weight_alpha = self.weight_alpha.float()/(self.num_users-1)
    ...
    def calculate_weight(self):
        ...
        for self_idx in range(self.num_users): #对某一model，计算其softmax后的weight
            weight_tmp = torch.zeros(self.num_users-1)
            weight_tmp = weight_tmp.cuda()
            idx_count = 0
            for teacher_idx in range(self.num_users):
                if self_idx == teacher_idx:
                    continue
                weight_tmp[idx_count] = self.weight_alpha[self_idx][teacher_idx]
                idx_count += 1
            weight_local = nn.functional.softmax(weight_tmp)
            idx_count = 0
            for teacher_idx in range(self.num_users): #对某一model，计算其他所有model的logits
                if self_idx == teacher_idx:
                    continue
                with torch.no_grad():
                    self.weight_alpha[self_idx][teacher_idx] = weight_local[idx_count]
                idx_count += 1                     
                    #self.weight_alpha[self_idx] = nn.functional.softmax(self.weight_alpha[self_idx]*10)                               
        self.np_weight_alpha = self.weight_alpha.cpu().clone().detach().numpy()
    ...
    def evaluate(self):
        ...
        with open("./results/"+'{}.txt'.format('evaluate'), 'a') as f:
